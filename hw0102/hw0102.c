...
int judgeStringInsert(char *str1, char *str2, int pos, const char *targetFile, int qNum){
    ...
    FILE *file2 = fopen("stuAns.txt", "r"); // stu ans file
    if(file2 == NULL){
        fclose(file1);
        fclose(file2);
        return pt; 
...
int main() {
    ...
    for(int i=1;i<=6;++i){
        ...
        idx[1] = '\0';
        idx[0] = i + '0';
        const char *file_name1 = stringCat("./sunnyCase/sunnyCase" , idx , "str1.txt");
        const char *file_name2 = stringCat("./sunnyCase/sunnyCase" , idx , "str2.txt");
        const char *ans_file_name  = stringCat("./ans/ans" , idx , ".txt");
        // printf("%s\n", file_name1);
        // printf("%s\n", file_name2);
        // file 1
    ...
    for(int i=1;i<=4;++i){
        ...
        const char *file_name1 = stringCat("./otherCase/otherCase" , idxT , "str1.txt");
        const char *file_name2 = stringCat("./otherCase/otherCase" , idxT , "str2.txt");
        char *ans_file_name; 
        if(i<4){
            ans_file_name = stringCat("./ans/ans" , idxA , ".txt");
        }
        else{
            ans_file_name = stringCat("./ans/ans" , "10" , ".txt");
        }
        // printf("%s\n", file_name1);
        // printf("%s\n", file_name2);
        // printf("%s\n", ans_file_name);
